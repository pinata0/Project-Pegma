using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

public class RainController : MonoBehaviour
{
    [Range(0, 1f)]
    public float masterIntensity = 1f;
    [Range(0, 1f)]
    public float rainIntensity = 1f;
    [Range(0, 1f)]
    public float windIntensity = 1f;
    [Range(0, 1f)]
    public float fogIntensity = 1f;
    [Range(0, 1f)]
    public float lightningIntensity = 1f;
    public bool autoUpdate;

    public ParticleSystem rainPart;
    public ParticleSystem windPart;
    public ParticleSystem lightningPart;
    public ParticleSystem fogPart;

    private ParticleSystem.EmissionModule rainEmission;
    private ParticleSystem.ForceOverLifetimeModule rainForce;
    private ParticleSystem.EmissionModule windEmission;
    private ParticleSystem.MainModule windMain;
    private ParticleSystem.EmissionModule lightningEmission;
    private ParticleSystem.MainModule lightningMain;
    private ParticleSystem.EmissionModule fogEmission;

    void Awake()
    {
        rainEmission = rainPart.emission;
        rainForce = rainPart.forceOverLifetime;
        windEmission = windPart.emission;
        windMain = windPart.main;
        lightningEmission = lightningPart.emission;
        lightningMain = lightningPart.main;
        fogEmission = fogPart.emission;
        UpdateAll();
    }

    void Update()
    {
        if (autoUpdate)
            UpdateAll();
    }

    void UpdateAll()
    {
        ParticleSystem.EmissionModule rainEmissionModule = rainEmission;
        rainEmissionModule.rateOverTime = new ParticleSystem.MinMaxCurve(200f * masterIntensity * rainIntensity);

        ParticleSystem.MinMaxCurve rainForceCurveX = new ParticleSystem.MinMaxCurve(-25f * windIntensity * masterIntensity, (-3 - 30f * windIntensity) * masterIntensity);
        rainForce.x = rainForceCurveX;

        ParticleSystem.EmissionModule windEmissionModule = windEmission;
        windEmissionModule.rateOverTime = new ParticleSystem.MinMaxCurve(5f * masterIntensity * (windIntensity + fogIntensity));

        ParticleSystem.MainModule windMainModule = windMain;
        windMainModule.startLifetime = 2f + 5f * (1f - windIntensity);
        windMainModule.startSpeed = new ParticleSystem.MinMaxCurve(15f * windIntensity, 25 * windIntensity);

        ParticleSystem.EmissionModule fogEmissionModule = fogEmission;
        fogEmissionModule.rateOverTime = new ParticleSystem.MinMaxCurve((1f + (rainIntensity + windIntensity) * 0.5f) * fogIntensity * masterIntensity);

        ParticleSystem.EmissionModule lightningEmissionModule = lightningEmission;
        if (rainIntensity * masterIntensity < 0.7f)
            lightningEmissionModule.rateOverTime = 0;
        else
            lightningEmissionModule.rateOverTime = new ParticleSystem.MinMaxCurve(lightningIntensity * masterIntensity * 0.4f);
    }

    public void OnMasterChanged(float value)
    {
        masterIntensity = value;
        UpdateAll();
    }
    public void OnRainChanged(float value)
    {
        rainIntensity = value;
        UpdateAll();
    }
    public void OnWindChanged(float value)
    {
        windIntensity = value;
        UpdateAll();
    }
    public void OnLightningChanged(float value)
    {
        lightningIntensity = value;
        UpdateAll();
    }
    public void OnFogChanged(float value)
    {
        fogIntensity = value;
        UpdateAll();
    }
}
